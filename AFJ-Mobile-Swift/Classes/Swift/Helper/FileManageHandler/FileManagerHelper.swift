...
class FileManagerHelper: NSObject {
    ...
    func getSubWithPath(_ fullPath: String) -> [AFJFileModel] {
        
        var result: [AFJFileModel] = []
        
        if let contents = try? useFileHandler.contentsOfDirectory(atPath: fullPath) {
            for content in contents {
                var isDir = ObjCBool(false)
                let path = fullPath.appending("/")+content
                
                if useFileHandler.fileExists(atPath: path, isDirectory: &isDir) {
                    result.append(AFJFileModel(path, isDirectory: isDir.boolValue))
                }
            }
        }
